defmodule LRU.Cache do
  ...
  defstruct max_size: 0,
            size: 0,
            value_by_key: Map.new(),
            timestamp_by_key: Map.new(),
            lru: :gb_trees.empty()
  ...
  def get(%LRU.Cache{} = cache, key) do
    ...
    case Map.has_key?(cache.value_by_key, key) do
      # return nil - consistent with Map API
      false ->
        {nil, cache}
  ...
  def put(%LRU.Cache{} = cache, key, value) when not is_nil(key) do
    ...
    case Map.has_key?(cache.value_by_key, key) do
      ...
      true ->
        ...
        cond do
          # Pre-existing key has new value
          value != prior_value ->
            # Proceed with cache insertion
            values = Map.put(cache.value_by_key, key, value)
  ...
  defp update_timestamp(%LRU.Cache{} = cache, key, current_timestamp)
    ...
    {prev, timestamps} =
      Map.get_and_update(cache.timestamp_by_key, key, fn t ->
    ...
    case :gb_trees.lookup(prev_lru_key, cache.lru) do
      # If key-value has been accessed before or initially seeded
      {:value, _lru_value} ->
  ...
  defp lru_discard(%LRU.Cache{} = cache) do
    ...
    %LRU.Cache{cache | size: size, lru: lru, value_by_key: values, timestamp_by_key: timestamps}
  end
end
